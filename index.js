...
function generateUniqueId() {
  ...
  return Date.now().toString() + Math.floor(Math.random() * 500).toString();
}
...
function addTodo() {
  ...
  if (text.trim() === "" || text === null) {console.log("To-do text cannot be empty.");
    return;
  }
  let newTodo = {
    id: generateUniqueId(),
  ...
  console.log('To-do "${text}" has been succsessfully.');
}
// belom
function markTodoCompleted() {
  ...
  listTodos()
  let todoNumber = prompt("Enter the number of the to-do to mark as completed: ");
...
function listTodos() {
 ...
 todos.forEach((todo, index) => {let status = todo.isCompleted ? "[DONE]" : "[ACTIVE]";
...
function runTodoApp() {
  ...
  let command = prompt("Enter a command inside the square brackets: ").toLowerCase();
  switch (command) {
  case "add": addTodo();
  break;
  case "complete":markTodoCompleted();
  break;
